...
assert_grep setup.sh 'NVIDIA_BASE_URL=.*https://integrate\.api\.nvidia\.com/v1' "engine defaults NVIDIA to the official NIM API base"
assert_grep setup.sh 'model_list: \[\]' "installation starts with no selected models"
assert_grep setup.sh 'PROVIDER_REG_TYPE="nvidia-nim"' "engine registers NVIDIA provider type"
assert_nogrep setup.sh 'NVIDIA_DEFAULT_MODEL|model_name:|gpt-4o|claude-sonnet|openrouter/auto' "installer does not select hard-coded models"
...
assert_nogrep scripts/register-provider.cjs 'SEED_MODELS' "provider registration does not seed models"
...
assert_grep src/components/dashboard/panels/SessionLauncherPanel.tsx '\$HOME/\.local/bin/clawnex run \$\{selectedHarness\.id\}' "coding-session launcher copies the stable installed CLI path"
