...
assert_grep setup.sh 'Local model[[:space:]]+— LM Studio or Ollama' "engine provider menu exposes local models"
assert_grep setup.sh 'Ollama[[:space:]]+— http://localhost:11434/v1' "engine local provider menu exposes Ollama"
assert_grep setup.sh 'PROVIDER_REG_MODEL_ID="\$LOCAL_MODEL_NAME"' "engine registers selected local model"
...
assert_grep install.sh '\-\-provider-url\)' "has --provider-url automation flag for local providers"
assert_grep install.sh '\-\-provider-model\)' "has --provider-model automation flag for local providers"
assert_grep install.sh 'Local model \(LM Studio/Ollama\)' "top-level provider menu exposes local models"
assert_grep install.sh 'Ollama[[:space:]]+— http://localhost:11434/v1' "top-level local provider menu exposes Ollama"
assert_grep install.sh 'CLAWNEX_ANSWER_LOCAL_PROVIDER_SELECT' "orchestrator preseeds local provider type into setup.sh"
assert_grep install.sh 'CLAWNEX_ANSWER_LOCAL_MODEL_NAME' "orchestrator preseeds local model name into setup.sh"
...
assert_grep scripts/register-provider.cjs 'PROVIDER_MODEL_ID' "provider registration can seed local model id"
assert_grep litellm/config.template.yaml 'LM Studio / Ollama' "LiteLLM template documents local LM Studio/Ollama endpoints"
