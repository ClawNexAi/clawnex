...
- UI consistency rule: when two dashboard panels expose the same kind of workflow, keep command/action buttons in the same structural placement. Peer actions belong in one shared command row, wrapping only when the viewport truly runs out of width. Do not split peer actions into separate rows or lower status cards unless there is a deliberate product reason documented in the UI.
