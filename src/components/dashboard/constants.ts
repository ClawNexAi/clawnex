...
export const PANEL_HELP: Record<TabId, { title: string; desc: string; metrics: string[]; actions: string[]; related: TabId[] }> = {
  ...
  securityPosture: {
    ...
    desc: "Shows your security grade based on Clawkeeper system hardening checks. Covers prerequisites, installation, host hardening, network, and security audit categories. Each check is scored and graded A-F. If Clawkeeper isn't installed yet, install it directly from the Welcome Wizard (Fleet Command) or from Configuration → Updates — this panel will start reporting as soon as the first scan finishes.",
    ...
    actions: ["Click Run Scan to execute a fresh security audit", "Install Clawkeeper from the Welcome Wizard or Configuration → Updates if it's missing", "Expand Hardening Report to see failed checks by tier", "Review Remediation Suggestions for prioritized fixes"],
...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: "Infrastructure & deployment",
    definition: "Letter grade (A–F) from Clawkeeper that summarizes the host's security posture across prerequisites, host hardening, network, and audit checks. Lower grade = more remediation needed.",
