...
export const PANEL_HELP: Record<TabId, { title: string; desc: string; metrics: string[]; actions: string[]; related: TabId[] }> = {
  ...
  configuration: {
    title: "Configuration",
    desc: "Platform settings hub organized into six collapsible groups: AI & Models, Fleet & Routing, Shield & Detection, Access Control, Integrations, and System. Fleet Connectors registers OpenClaw, Hermes, AnythingLLM, OpenCode, Pi, Codex, and Claude Code on this host. Supported routing panels use the same operator-controlled sequence: select routes, review exact changes, apply, send a new request, verify fresh evidence, and restore the original direct connection when needed. AnythingLLM model access is confirmed with the amber Test model action before a route can be applied. Configuration also manages providers, Shield behavior, authentication, operators, API keys, mail, updates, retention, UI preferences, and system lifecycle actions.",
