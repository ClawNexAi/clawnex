...
export const PANEL_HELP: Record<TabId, { title: string; desc: string; metrics: string[]; actions: string[]; related: TabId[] }> = {
  ...
  help: {
    ...
    metrics: ["Panels — every visible dashboard panel, organized by sidebar group", "Tooltips — contextual explanations controlled by the TIPS button", "Keyboard Shortcuts — Escape for tooltips, Enter for form submission, browser defaults", "Documentation — operator guides in Help plus policy and register documents in Governance"],
  ...
  configuration: {
    title: "Configuration",
    desc: "Platform settings hub organized into six collapsible groups: AI & Models, Fleet & Routing, Shield & Detection, Access Control, Integrations, and System. Fleet Connectors registers OpenClaw, Hermes, AnythingLLM, OpenCode, Pi, Codex, and Claude Code on this host. Supported routing panels use the same operator-controlled sequence: select routes, test each affected model beside its routing row, review exact changes, apply, send a new request, verify fresh evidence, and restore the original direct connection when needed. The shared amber Test model action becomes green Model verified after a successful proxy test. Configuration also manages providers, Shield behavior, authentication, operators, API keys, mail, updates, retention, UI preferences, and system lifecycle actions.",
    metrics: ["Connector health — connected, unavailable, or not configured", "Route state — direct, selected, managed, or conflicting", "Model readiness — Test model or Model verified", "Block Mode — OBSERVE or BLOCK", "Retention — per-category data retention periods", "Operators — active and locked accounts with role badges", "Updates — installed and available component versions"],
    actions: ["Register or remove a supported fleet connector", "Test an exact model before applying a route", "Review, apply, verify, and restore supported routing changes", "Add model providers and discover their models", "Manage Shield settings, correlation rules, and threat-score weights", "Manage operators, authentication methods, sessions, and API keys", "Configure mail, scheduled reports, updates, retention, HTTPS, and UI preferences", "Archive, migrate, purge, or uninstall through reviewed system actions"],
