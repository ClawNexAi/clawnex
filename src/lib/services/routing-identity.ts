 ...
 */
export function createRoutingIdentity(connector: 'openclaw' | 'hermes', sourceId: string): { token: string; hash: string } | null {
...
export function prepareIdentityHeader(headers: Record<string, unknown>, ownership: RoutingIdentityOwnership,
  connector: 'openclaw' | 'hermes', sourceId: string): string | null {
