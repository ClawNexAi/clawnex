 ...
 */
export function createRoutingIdentity(connector: 'openclaw' | 'hermes' | 'opencode' | 'pi' | 'codex' | 'claude' | 'anythingllm', sourceId: string): { token: string; hash: string } | null {
...
export function prepareIdentityHeader(headers: Record<string, unknown>, ownership: RoutingIdentityOwnership,
  connector: 'openclaw' | 'hermes' | 'opencode' | 'pi' | 'codex' | 'claude', sourceId: string): string | null {
