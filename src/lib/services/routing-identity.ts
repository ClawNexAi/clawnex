 ...
 */
export function createRoutingIdentity(connector: 'openclaw' | 'hermes' | 'opencode' | 'anythingllm', sourceId: string): { token: string; hash: string } | null {
...
export function prepareIdentityHeader(headers: Record<string, unknown>, ownership: RoutingIdentityOwnership,
  connector: 'openclaw' | 'hermes' | 'opencode', sourceId: string): string | null {
