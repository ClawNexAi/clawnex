...
/** Resolve an agent's upstream model id to the exact alias exposed by LiteLLM. */
export function resolveConfiguredProxyModel(
  ...
  const exact = eligible.filter(model => model.model_id === modelId);
  if (exact.length === 1) return { providerId: exact[0].provider_id, modelAlias: exact[0].model_id };
  const qualified = eligible.filter(model => {
    const provider = getProvider(model.provider_id);
    return provider != null && model.model_id === `${provider.type}/${modelId}`;
  });
  return qualified.length === 1
    ? { providerId: qualified[0].provider_id, modelAlias: qualified[0].model_id }
    : null;
}
