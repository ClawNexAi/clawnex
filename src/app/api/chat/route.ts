...
- **Model Pricing** is refreshable intelligence, same pattern as CVE intel. ClawNex ships with a bundled LiteLLM price snapshot that seeds the \`model_prices\` DB on first boot. Operators refresh from [Configuration] → Updates → Model Pricing → Update Now, which pulls from LiteLLM's GitHub at the exact pinned stable release tag only (never upstream main). They can also set an auto-sync schedule (hours interval) and a stale threshold (days). This is how the Token & Cost Intel panel stays current as new models launch.
...
const KEYWORD_RESPONSES: Array<{ keywords: string[]; response: string }> = [
  ...
  },
  {
    ...
    response: "**Connector → LiteLLM routing.** For ClawNex's Prompt Shield to scan agent traffic in real time, selected OpenClaw providers or Hermes custom providers must route through the LiteLLM proxy at `http://127.0.0.1:4001/v1`.\n\n1. Use **[Configuration] → OpenClaw Routing** for OpenClaw providers/models.\n2. Use **[Configuration] → Hermes Routing** for writable Hermes custom providers/models.\n3. Tick the rows you want protected, then click **Apply OpenClaw Routing** for OpenClaw or **Save Hermes Wire** for Hermes.\n4. Use **Revert Hermes Wire** to undo ClawNex-managed Hermes provider edits, and **Restart Gateway** so Hermes reloads `config.yaml`.\n5. Routing is enforced at provider endpoint level: selecting a model routes that model's provider endpoint, so sibling models on the same provider follow the same route.\n6. Watch the header **Updates** badge for connector-routing drift when providers/models are added or removed.\n\nHermes OAuth/session-bound and watcher-only rows remain read-only retrospective inventory. The legacy **Wire LiteLLM** action remains available only for OpenClaw compatibility.",
  },
  {
