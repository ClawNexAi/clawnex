...
The peer Stat tile lift (also `aeade4b`) is **not** opt-in — every `Stat` consumer reads as an elevated panel by default via the new `glassPanelNested` / `glassPanelNested2` / `glassBorderCyanStrong` theme tokens. Visual lift was verified during the release QA pass.
