...
Alpha. Single-maintainer open-source project released under Apache 2.0. **Not yet pilot-ready for regulated enterprise data.** Reference deployments are local dev host (local dev), QA host (VPS provider QA, currently LIVE at https://qa.clawnexai.com running v0.15.1-alpha), and test host (Mac dev-box reserve).
