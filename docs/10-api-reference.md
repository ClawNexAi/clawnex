...
**Localhost bypass semantics:** When `RBAC_ENABLED=false`, requests from `127.0.0.1` are accepted without an authenticated session. As of P0-A 2026-05-14, `requireLocalhost()` also enforces an **Origin/Referer same-host match** on every mutating method (POST/PUT/PATCH/DELETE). A browser tab loaded on `evil.com` that POSTs cross-origin to the local dashboard is refused with `403 CSRF: origin mismatch` even though the source IP is loopback. Safe methods (GET/HEAD/OPTIONS) and non-browser callers with no `Origin` and no `Referer` are not subject to the Origin check. When `RBAC_ENABLED=true`, every internal API route requires a valid session cookie regardless of origin, and the session path runs the same shared Origin-match helper as Layer 1 of CSRF defense.
...
> **2026-05-14 (H1 DAST remediation — login timing oracle).** Every 401 return is wrapped by a `MIN_LOGIN_FAILURE_MS = 2000` response-time floor. Failed logins for existing users used to take ~4× the wall-clock of failed logins for non-existent users (admin@2638ms vs nonexistent@651ms) because only the real-user path ran `incrementFailedLogin` + the lockout-decay computation. Every failure response now sleeps until at least 2000ms elapsed since request start before responding; the success path is exempt. Live delta on QA after fix = 4ms.
...
> **2026-05-14 (H4 DAST remediation).** `operatorCount` is **only** included in the response for authenticated callers. Anonymous probes previously learned the exact account count, which was useful enumeration intelligence ("only one admin → target them"). The boolean `needsSetup` is sufficient for the login page to decide setup-vs-login.
...
| 1.6 | 2026-04-22 | ClawNex Engineering | v0.6.2-alpha hardening pass: `/api/trust-audit` now returns the wrapped `{report, meta: {last_run, duration_ms, cached}}` shape and `?refresh=true` forces a fresh run; previous flat shape is no longer returned. Health version string bumped to `0.6.2-alpha`. API perf pass adds 4 indexes (documented in §22). MCP tool invocations are now audit-logged. See `docs/security-audit-2026-04-22.md` and CHANGELOG §[0.6.2-alpha]. |
