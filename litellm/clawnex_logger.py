...
def _remember_routing_identity(auth, data):
    ...
    if connector not in {"openclaw", "hermes", "opencode", "pi", "codex", "anythingllm"} or not isinstance(source_id, str) or not 0 < len(source_id) <= 200:
        return
...
def _source_for(model, data=None):
    ...
    if isinstance(metadata, dict):
        ...
        if connector in {"hermes", "openclaw", "opencode", "pi", "codex"}:
            return connector
